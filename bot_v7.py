...
# importa la capa DB centralizada
import database
...
# Helper wrappers mapping to database.py
...
# Reuse same logic for add_balance, daily reset, etc., but using DB functions
...
def give_daily_bonus_db(user_id):
    ...
    if now - int(user.get("last_daily", 0)) >= 86400:
        ...
        # no hace falta save_user_db(user) aquí
        return DAILY_BONUS
    return 0.0
...
# --- Handlers (igual que tu versión, usando funciones del DB) ---
...
if __name__ == "__main__":
    ...
    print("Bot v7 corriendo... presiona Ctrl+C para detener.")
